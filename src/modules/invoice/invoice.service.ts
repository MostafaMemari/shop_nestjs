...
export class InvoiceService {
  ...
  async createInvoice(createInvoiceDto: CreateInvoiceDto): Promise<Invoice> {
    ...
    invoice.total_amount = invoice.items.reduce(
      (sum, item) => sum + (item.status === ItemStatus.SUCCESS ? item.price * item.quantity : 0),
      0,
    );
